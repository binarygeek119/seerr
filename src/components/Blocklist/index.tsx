      ...
      {!data ? (
        ...
        data.results.map((item: BlocklistItem) => {
          return (
            <div
              className="py-2"
              key={`request-list-${item.mediaType}-${item.tmdbId ?? item.mbId}`}
            >
...
const BlocklistedItem = ({ item, revalidateList }: BlocklistedItemProps) => {
  ...
  const removeFromBlocklist = async (
    ...
    try {
      ...
      await axios.delete(
        `/api/v1/blocklist/${encodeURIComponent(String(id))}?mediaType=${item.mediaType}`
      );
        ...
        <div className="z-10 ml-4 mt-4 flex w-full flex-col justify-center overflow-hidden pr-4 text-sm sm:ml-2 sm:mt-0 xl:flex-1 xl:pr-0">
          ...
          <div className="card-field">
            {item.mediaType === 'movie' ? (
              ...
              <div className="pointer-events-none z-40 self-start rounded-full border border-green-500 bg-green-600/80 shadow-md">
                <div className="flex h-4 items-center px-2 py-2 text-center text-xs font-medium uppercase tracking-wider text-white sm:h-5">
