...
:::info
This fork’s Docker image is on Docker Hub: [binarygeek119/seerr](https://hub.docker.com/r/binarygeek119/seerr). Official upstream images remain at [seerr/seerr](https://hub.docker.com/r/seerr/seerr).
...
docker run -d \
  ...
  --health-interval 15s \
  --health-retries 3 \
  ghcr.io/binarygeek119/seerr:latest
```
...
```bash
docker pull ghcr.io/binarygeek119/seerr:latest
```
...
services:
  seerr:
    image: ghcr.io/binarygeek119/seerr:latest
    init: true
