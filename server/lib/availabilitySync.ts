...
class AvailabilitySync {
  ...
  async run() {
    ...
    this.enable4kMovie = settings.radarr.some((server) => server.is4k);
    this.enable4kShow = settings.sonarr.some((server) => server.is4k);
  ...
  private async mediaBookExistsInAudiobookshelf(
    media: Media
  ): Promise<boolean> {
