...
/**
 * Repair migration: AddMusicSupport added mbId, but AddPerformanceIndexes recreated
 * media/watchlist/blocklist without it. Restores columns for existing SQLite DBs.
 */
  ...
  private async ensureMbIdColumn(
    ...
    if (!table.findColumnByName('mbId')) {
      await queryRunner.query(
        `ALTER TABLE "${tableName}" ADD COLUMN "mbId" varchar`
      );
    }

    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "${indexName}" ON "${tableName}" ("mbId")`
    );
  }
